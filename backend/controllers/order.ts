...
export const createOrder = async (req: Request, res: Response) => {
    try {
        ...
        for (let i = 0; i < cartItems.length; i++) {
            ...
            tempStoreItem.quantity = (<number>cartItems[i].cart_item.quantity - <number>cartItems[i].quantity);
            // Update the store inventory repo given the number id and the field to augment.
            await storeInventoryRepository.update(<number>cartItems[i].cart_item.item_id, tempStoreItem);
        }
...
export const cancelOrder = async (req: Request, res: Response) => {
    try {
        ...
        const orderToCancel: Order | null =
        ...
        for (let i = 0; i < orderedItems.length; i++) {
            ...
            // Construct the partial object:
            var tempStoreItem = new InventoryItem();
            tempStoreItem.quantity = (<number>orderedItems[i].inventory_item.quantity + <number>orderedItems[i].quantity);
            // Update the store inventory repo given the number id and the field to augment.
            await storeInventoryRepository.update(<number>orderedItems[i].inventory_item.item_id, tempStoreItem);
